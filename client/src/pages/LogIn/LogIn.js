...
function LogIn() {
    ...
    useEffect(() => {
        if (user && Object.keys(user).length) navigate("/")
    }, [user])
    ...
    return (
        <>
            ...
            <AuthForm onSubmit={submitHandler}>
                ...
                <Link to="#" className="text-black-50 text-start d-block fs-9 accent-clr-hover">
                    Forget your password?
                </Link>
                <Button val="Log In" />
                <p className="text-black-50 mt-3 fs-9">Don't Have an Account?
                    <Link to="/signup" className="text-clr-primary accent-clr-hover"> Create account</Link>
                </p>
            </AuthForm>
        </>
    )
